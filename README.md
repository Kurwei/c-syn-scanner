...
**Response Parsing:** Actively listens to the network interface via `recvfrom()` to catch and parse returning `SYN-ACK` (Open), `RST` (Closed) flags and if there is no response scanner will skip the port with a (filtered) flag.
...
## USAGE
The scanner requires exactly five arguments: Source IP, Source Port, Target IP, and Target Port.  
sudo ./syn_scanner <source_ip> <source_port> <target_ip> <starting_port> <ending_port>
